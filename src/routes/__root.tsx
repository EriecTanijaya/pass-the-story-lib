...
// const getCurrentUserApiBak = createServerFn({ method: "GET" }).handler(
// 	async () => {
...
// );
//
const getCurrentUserApi = createServerFn({ method: "GET" }).handler(async () => {
  ...
  //TODO: need domain object validation here
  const user: User = {
    id: session.data.id,
    name: session.data.name as string,
    role: session.data.role as Role,
    profileImageUrl: session.data.profileImageUrl,
  };
