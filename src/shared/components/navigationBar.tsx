...
import { getAuthSession } from "@/features/auth/getAuthSession/api/getAuthSession";
import { Button } from "./ui/button";
import { NavigationMenu, NavigationMenuItem, NavigationMenuList } from "./ui/navigation-menu";
import { Separator } from "./ui/separator";

const signOutApi = createServerFn({ method: "POST" }).handler(async () => {
...
export function NavigationBar({ user }: NavigationBarProps) {
  ...
  async function handleSignOut() {
    ...
    if (!res.success) {
      toast(`Having trouble signing out`, { position: "bottom-center" });
    }
    ...
    <header className="bg-background/80 sticky top-0 z-1 backdrop-blur-md border-b border-b-muted-foreground/50 px-7 xl:px-24">
      <NavigationMenu className="h-16 justify-between max-w-full">
        ...
        <NavigationMenuList className="hidden md:flex ">
          {(() => {
            const navMenus = user?.role === "admin" ? adminNavMenus : memberNavMenus;
              ...
              return (
                <NavigationMenuItem key={nav.name}>
                  <Link to={nav.link} activeOptions={{ exact: true }}>
                    {({ isActive }) => {
                      return <Button variant={isActive ? "secondary" : "ghost"}>{nav.name}</Button>;
                    }}
                  </Link>
                </NavigationMenuItem>
              );
          ...
          return (
            <div className="hidden md:flex items-center gap-3">
              {user.profileImageUrl ? (
                <img src={user.profileImageUrl} alt="profile" className="size-7 rounded-full" />
              ) : (
        ...
        <div className="md:hidden flex">
          <button type="button" onClick={() => setIsOpen(!isOpen)}>
            {!isOpen ? <ListIcon className="size-6" /> : <XIcon className="size-6" />}
          </button>
        </div>
      </NavigationMenu>
          ...
          return navMenus.map((nav) => {
            return (
              ...
              >
                {({ isActive }) => {
                  return (
                    <Button variant={isActive ? "secondary" : "ghost"} className="w-full">
                      {nav.name}
                    </Button>
                  );
                }}
